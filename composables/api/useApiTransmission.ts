...
export function useApiTransmission() {
  const findAll = async (
    params: params,
    options?: Partial<ApiFetchOptions<ApiUomListResponse>>,
  ) => {
    return await useApi<ApiUomListResponse>(API_ENDPOINT.UOM.TRANSMISSION, {
      method: 'GET',
      params,
      ...options,
      onSuccess: ({ body }) => {
  ...
  const create = async (
    ...
    return await useApi<ApiUomResponse>(API_ENDPOINT.UOM.TRANSMISSION, {
      method: 'POST',
      body: payload,
      ...options,
      watch: false,
  ...
  const update = async (
    ...
    return await useApi<ApiUomResponse>(`${API_ENDPOINT.UOM.TRANSMISSION}/${id}`, {
      method: 'PATCH',
      body: payload,
      ...options,
      watch: false,
